  ...
  try {
    ...
    if (!presignedResponse.ok) {
      ...
      return {
        ...
        error:
          error.error ||
          `HTTP ${presignedResponse.status}: ${presignedResponse.statusText}`,
      };
    }
    ...
    if (!confirmResponse.ok) {
      ...
      return {
        success: false,
        error: "Evidence confirmation failed. Please retry the upload.",
      };
    } else {
    }
...
export function validateFileForUpload(file: File): {
  ...
  if (!ALLOWED_TYPES.includes(file.type)) {
    return {
      valid: false,
      error:
        "Only JPEG, PNG, WebP images and MP4, QuickTime, WebM videos are allowed",
    };
  }
