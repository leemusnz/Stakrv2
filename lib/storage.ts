...
export function validateFile(file: File): { valid: boolean; error?: string } {
  ...
  if (
    file.name.match(
      /^(stock|shutterstock|getty|adobe|istockphoto|unsplash|pexels|fake|test|sample|demo|generated|ai_?generated)/i,
    )
  ) {
    return {
      valid: false,
...
/** Confirm metadata from S3, then copy to a server-only immutable evidence key. */
export async function confirmEvidenceObject(
  key: string,
  user: string,
  challenge: string,
) {
  ...
  if (
    parts.length !== 5 ||
    parts[0] !== "verification-files" ||
    !["images", "videos"].includes(parts[1]) ||
    parts[2] !== challenge ||
    parts[3] !== user ||
    /\.\.|[\\\u0000]/.test(key)
  )
  ...
  const object = await client.send(
    new HeadObjectCommand({ Bucket: STORAGE_CONFIG.AWS_BUCKET_NAME, Key: key }),
  );
  ...
  if (
    !object.ETag ||
    size < 1 ||
    size > STORAGE_CONFIG.MAX_FILE_SIZE ||
    ![
      ...STORAGE_CONFIG.ALLOWED_IMAGE_TYPES,
      ...STORAGE_CONFIG.ALLOWED_VIDEO_TYPES,
    ].includes(type) ||
    object.Metadata?.["user-id"] !== user ||
    object.Metadata?.["challenge-id"] !== challenge
  )
    throw new Error("Evidence metadata does not match this submission");
  const storageKey = `confirmed-evidence/${challenge}/${user}/${randomUUID()}.${getFileExtension(type)}`;
  await client.send(
    new CopyObjectCommand({
      Bucket: STORAGE_CONFIG.AWS_BUCKET_NAME,
      Key: storageKey,
      CopySource: encodeURIComponent(
        STORAGE_CONFIG.AWS_BUCKET_NAME + "/" + key,
      ),
      CopySourceIfMatch: object.ETag,
    }),
  );
  return { storageKey, contentType: type, byteSize: size };
}
...
export async function uploadToCloudinary(
  ...
  if (
    !STORAGE_CONFIG.CLOUDINARY_CLOUD_NAME ||
    !STORAGE_CONFIG.CLOUDINARY_API_KEY ||
    !STORAGE_CONFIG.CLOUDINARY_API_SECRET
  ) {
