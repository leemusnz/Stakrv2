...
export async function joinChallenge(
  ...
  return withTransaction(async (sql) => {
    ...
    const challenges =
      await sql`SELECT c.* FROM challenges c WHERE c.id = ${challengeId} FOR UPDATE`;
    if (!challenges[0]) throw new JoinError("Challenge not found", 404);
    const challenge = challenges[0];
    if (challenge.lifecycle_version !== 1)
      throw new JoinError(
        "This challenge needs a rules and credit review before accepting entries",
        409,
      );
    ...
    `;
    if (!eligibility[0]?.can_join)
      throw new JoinError("Joining closes when the challenge starts");

    const existing = await sql`
      SELECT id FROM challenge_participants WHERE challenge_id = ${challengeId} AND user_id = ${userId}
    `;
    if (existing.length)
      throw new JoinError("You have already joined this challenge", 409);
    const counts =
      await sql`SELECT COUNT(*) AS count FROM challenge_participants WHERE challenge_id = ${challengeId}`;
    const count = Number(counts[0].count);
    if (
      challenge.max_participants != null &&
      count >= Number(challenge.max_participants)
    ) {
    ...
    if (challenge.allow_points_only)
      throw new JoinError(
        "XP-only challenges are not available in the credits MVP",
        409,
      );
    const pointsOnly = false;
    if (input.pointsOnly !== pointsOnly) {
      throw new JoinError(
        pointsOnly
          ? "This challenge uses points only"
          : "This challenge requires a credit stake",
      );
    }
    let proofRequirements = challenge.proof_requirements ?? {};
    if (typeof proofRequirements === "string") {
      try {
        proofRequirements = JSON.parse(proofRequirements);
      } catch {
        throw new JoinError(
          "Challenge payment configuration needs review",
          409,
        );
      }
    }
    if (
      !proofRequirements ||
      typeof proofRequirements !== "object" ||
      Array.isArray(proofRequirements)
    ) {
    ...
    if (!pointsOnly && currency !== "CREDITS") {
      throw new JoinError(
        "Cash challenge entry is not available in the credits MVP",
        403,
      );
    }
    // These modes lack the policy/settlement needed for a fair MVP. Do not sell them.
    if (input.insurancePurchased)
      throw new JoinError("Challenge insurance is not available in the MVP");
    if (challenge.enable_team_mode)
      throw new JoinError("Team challenges are not available in the MVP", 409);
    if (input.referralCode || input.teamPreference)
      throw new JoinError(
        "Referrals and team preferences are not available in the MVP",
      );

    let stakeCents = 0;
    if (!pointsOnly) {
      try {
        stakeCents = toCreditCents(input.stakeAmount);
      } catch {
        throw new JoinError(
          "Stake must be a positive credit amount with at most two decimal places",
        );
      }
      if (
        stakeCents <= 0 ||
        stakeCents < toCreditCents(challenge.min_stake) ||
        stakeCents > toCreditCents(challenge.max_stake)
      ) {
        throw new JoinError(
          `Stake must be between ${challenge.min_stake} and ${challenge.max_stake} credits`,
        );
      }
      ...
      if (tiers && !tiers.map(toCreditCents).includes(stakeCents))
        throw new JoinError("Choose one of the challenge stake tiers");
    }
    ...
    if (!Number.isSafeInteger(totalCents))
      throw new JoinError("Total credit amount is too large");

    // Serialize balance checks even when this user joins DIFFERENT challenges.
    const users =
      await sql`SELECT credits FROM users WHERE id = ${userId} FOR UPDATE`;
    ...
    const audit = {
      participantId: participation[0].id,
      pointsOnly,
      stakeCents,
    ...
    `;
    const host = challenge.host_id
      ? await sql`SELECT name FROM users WHERE id = ${challenge.host_id}`
      : [];
    return {
      participation: {
      ...
      financial_breakdown: {
        stake_amount: stakeCents / 100,
        entry_fee: entryCents / 100,
        insurance_fee: 0,
        total_cost: totalCents / 100,
        remaining_credits: balanceCents / 100,
        reward_type: pointsOnly ? "XP" : "CREDITS",
      },
      challenge_info: {
        title: challenge.title,
        total_participants: count + 1,
        host_name: host[0]?.name ?? null,
