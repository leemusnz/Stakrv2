...
export type RewardDistributionMethod =
  | "winner-takes-all"
  | "equal-split"
  | "proportional";
...
export async function calculatePotentialReward(
  ...
  const estimatedTotalParticipants =
    parseInt(challenge.current_participants) + 1;
  const estimatedFailedStakes = avgStake * estimatedTotalParticipants * 0.3; // Assume 30% failure rate
  const availablePool =
    estimatedFailedStakes * (1 - parseFloat(challenge.failed_stake_cut) / 100) +
    parseFloat(challenge.host_contribution);
...
export async function calculateChallengeRewards(
  ...
  const challengeStats: ChallengeStats = {
    ...
    reward_distribution: (challenge.reward_distribution ||
      "equal-split") as RewardDistributionMethod,
    ...
    completion_rate:
      challenge.total_participants > 0
        ? (challenge.completed_participants / challenge.total_participants) *
          100
        : 0,
  };
  ...
  const platformRevenue = {
    ...
    failed_stake_cut:
      challengeStats.failed_stakes * (challengeStats.failed_stake_cut / 100),
    total: 0,
  };
  platformRevenue.total =
    platformRevenue.entry_fees + platformRevenue.failed_stake_cut;

  // Calculate the amount that goes to winners (remainder after platform cut)
  const failedStakesForWinners =
    challengeStats.failed_stakes - platformRevenue.failed_stake_cut;
  ...
  // Calculate summary
  const totalDistributed = participantRewards.reduce(
    (sum, r) => sum + r.net_reward,
    0,
  );
  const summary = {
    ...
    average_reward:
      participantRewards.length > 0
        ? totalDistributed / participantRewards.length
        : 0,
    max_reward:
      participantRewards.length > 0
        ? Math.max(...participantRewards.map((r) => r.net_reward))
        : 0,
    min_reward:
      participantRewards.length > 0
        ? Math.min(...participantRewards.map((r) => r.net_reward))
        : 0,
  };
...
function calculateRewardDistribution(
  ...
  const { reward_distribution, failed_stakes, host_contribution } =
    challengeStats;
  ...
  switch (reward_distribution) {
    ...
    default:
      throw new Error(
        `Unknown reward distribution method: ${reward_distribution}`,
      );
  }
}
...
function calculateWinnerTakesAll(
  ...
  const bonusPerParticipant =
    participants.length > 0 ? bonusPool / participants.length : 0;
  const hostContributionShare =
    participants.length > 0 ? hostContribution / participants.length : 0;
 ...
 */
/** @deprecated Settlement must use the locked, audited lifecycle service. */
export async function distributeRewards(
  _challengeId: string,
  _sqlOverride?: SqlTag,
): Promise<RewardDistributionResult> {
  throw new Error(
    "Legacy settlement is disabled; use processChallenge from challenge-engine",
  );
}
