...
export class EnhancedAIVerification {
  ...
  private static async verifyManualSubmission(
    ...
    const prompt = this.buildManualVerificationPrompt(
      challengeText,
      manualData,
    );
   ...
   */
  private static buildAutoSyncPrompt(
    request: EnhancedVerificationRequest,
  ): string {
...
`;
   ...
   */
  private static buildManualVerificationPrompt(
...
`;
  ...
  private static fallbackManualValidation(
    ...
    return {
      approved: false, // A failed verification must never approve evidence
      confidence: 50,
