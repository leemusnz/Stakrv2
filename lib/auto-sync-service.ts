  ...
  try {
    ...
    if (response.status === 401 || response.status === 403) {
      try {
        ...
        if (!refreshRes.ok) {
          ...
          return {
            success: false,
            error: "Strava token expired and refresh failed",
            provider: "strava",
          };
        }
        // Persist new tokens
        ...
        await sqlUpdate`
          ...
          SET api_credentials = ${JSON.stringify({
            ...
            refresh_token:
              refreshData.refresh_token || credentials.refresh_token,
        ...
        return {
          success: false,
          error: "Failed during Strava token refresh",
          provider: "strava",
        };
      }
    }
    ...
    activities.forEach((activity: any, index: number) => {
      ...
      const hoursDiff = Math.floor(
        (new Date().getTime() - activityDate.getTime()) / (1000 * 60 * 60),
      );
    });
    ...
    for (const activity of activities) {
      ...
      try {
        ...
        await sql`
          INSERT INTO proof_submissions (
            ...
            ${JSON.stringify({
              ...
              duration: activity.moving_time,
              duration_minutes:
                Math.round((activity.moving_time / 60) * 10) / 10,
              date: activity.start_date,
        ...
        console.error(
          `🤖 AI verification failed for activity ${activity.id}:`,
          aiError,
        );
...
export async function syncSpotifyData(
  ...
  try {
    ...
    if (educationalTracks.length > 0) {
      ...
      for (const item of educationalTracks.slice(0, 5)) {
...
export async function syncChallengeData(
  ...
  try {
    ...
    const challenges = await sql`
      SELECT lifecycle_version, title, description, proof_requirements, verification_type, ai_analysis
      FROM challenges 
    ...
    if (challenges[0].lifecycle_version === 1)
      return [
        {
          success: false,
          error: "This challenge requires neutral manual evidence review",
        },
      ];
    ...
    const challenge = {
      ...
      aiAnalysis: challenges[0].ai_analysis
        ? typeof challenges[0].ai_analysis === "string"
          ? JSON.parse(challenges[0].ai_analysis)
          : challenges[0].ai_analysis
        : null,
    };
    ...
    if (!requiresAutoSync(challenge)) {
      return [
        { success: true, syncedData: { message: "No auto-sync required" } },
      ];
    }
    ...
    for (const wearable of integrations.wearables) {
      try {
        const credentials =
          typeof wearable.api_credentials === "string"
    ...
    for (const app of integrations.apps) {
      try {
        const credentials =
          typeof app.api_credentials === "string"
    ...
    return results.length > 0
      ? results
      : [{ success: true, syncedData: { message: "No integrations to sync" } }];
  } catch (error) {
