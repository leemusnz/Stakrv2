...
const { distributeRewards } = jest.requireActual(
  "@/lib/reward-calculation",
) as typeof import("@/lib/reward-calculation");
...
beforeEach(() => {
  process.env.ENABLE_CASH_PAYMENTS = "true";
  calls.length = 0;
  dispatcher = () => [];
...
describe("P0-5: credits join writes participant + debit + ledger rows", () => {
  ...
  async function postJoin() {
    const request = new NextRequest(
      "http://localhost:3000/api/challenges/challenge-1/join",
      {
        method: "POST",
        body: JSON.stringify({
          stakeAmount: 50,
  ...
  it("issues all three writes in one request and the join succeeds", async () => {
    ...
    const ledgerInsert = calls.find((c) =>
  ...
  it("REGRESSION: ledger descriptions are bound parameters, never SQL literals", async () => {
    ...
    expect(ledgerInsert.values).toContain(
      "Stake locked for challenge: Test Challenge",
    );
    expect(ledgerInsert.values).toContain(
      "Entry fee for challenge: Test Challenge",
    );
  ...
  it("refuses insurance until its policy is implemented", async () => {
    ...
    expect(
      calls.some((c) => c.text.includes("INSERT INTO challenge_participants")),
    ).toBe(false);
  });

  it("returns success when the post-commit reward estimate is unavailable", async () => {
    dispatcher = joinDispatcher;
    rewardCalculationMock.calculatePotentialReward.mockRejectedValue(
      new Error("estimate unavailable"),
    );
    const response = await postJoin();
    expect(response.status).toBe(201);
    expect(
      (await response.json()).financial_breakdown.potential_reward,
    ).toBeNull();
  });
  ...
  it("returns 400 for malformed JSON without writing", async () => {
    const response = await joinPost(
      new NextRequest("http://localhost:3000/api/challenges/challenge-1/join", {
        method: "POST",
        body: "{",
      }),
      { params: Promise.resolve({ id: "challenge-1" }) },
...
describe("Legacy settlement is closed", () => {
  it("cannot write credits outside the canonical lifecycle", async () => {
    await expect(
      distributeRewards("challenge-1", mockSql as any),
    ).rejects.toThrow("Legacy settlement is disabled");
    expect(calls).toHaveLength(0);
  });
});
...
describe("P0-6: Stripe idempotency", () => {
  ...
  it("processes a checkout event once and ignores the duplicate delivery", async () => {
    ...
    const statusUpdates = calls.filter((c) =>
      c.text.includes("UPDATE transactions"),
    ).length;
