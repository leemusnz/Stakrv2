  ...
  describe("GET /api/challenges", () => {
    it("should return joinable challenges", async () => {
      const mockChallenges = [
        {
          ...
          thumbnail_url:
            "https://stakr-verification-files.s3.ap-southeast-2.amazonaws.com/thumbs/challenge-1.webp",
        },
      ];
      ...
      const request = new NextRequest(
        "http://localhost:3000/api/challenges?status=joinable",
      );
    ...
    it("should handle limit parameter", async () => {
      ...
      const request = new NextRequest(
        "http://localhost:3000/api/challenges?limit=5",
      );
      const response = await GET(request);

      expect(response.status).toBe(200);
      expect(mockSql.mock.calls[0][0].join("?")).toContain("LIMIT ?");
      expect(mockSql.mock.calls[0]).toContain(5);
    });
...
describe("Social Feed API Integration", () => {
  ...
  describe("GET /api/social/feed", () => {
    ...
    it("should handle pagination parameters", async () => {
      ...
      const request = new NextRequest(
        "http://localhost:3000/api/social/feed?cursor=post-123&limit=20",
      );
      const response = await GET(request);
