...
jest.mock("@/lib/db", () => ({ createDbConnection: () => mockSql }));
...
beforeEach(() => {
  mockSql.mockImplementation(async (_strings: any, key: string) => [
    { file_key: key, storage_key: key },
  ]);
  process.env = {
    ...originalEnv,
...
describe("Image Proxy API Tests", () => {
  describe("GET /api/image-proxy", () => {
    it("should proxy S3 images successfully", async () => {
      const s3Url =
        "https://stakr-verification-files.s3.ap-southeast-2.amazonaws.com/verification-files/images/challenge-123/thumbnail.webp";
    ...
    it("should handle S3 errors gracefully", async () => {
      const s3Url =
        "https://stakr-verification-files.s3.ap-southeast-2.amazonaws.com/verification-files/images/nonexistent.webp";
    ...
    it("should handle different image content types", async () => {
      const testCases = [
        {
          url: "https://stakr-verification-files.s3.ap-southeast-2.amazonaws.com/image.jpg",
          contentType: "image/jpeg",
        },
        {
          url: "https://stakr-verification-files.s3.ap-southeast-2.amazonaws.com/image.png",
          contentType: "image/png",
        },
        {
          url: "https://stakr-verification-files.s3.ap-southeast-2.amazonaws.com/image.webp",
          contentType: "image/webp",
        },
        {
          url: "https://stakr-verification-files.s3.ap-southeast-2.amazonaws.com/image.gif",
        ...
        mockGetObject.mockResolvedValueOnce({
          Body: {
            transformToByteArray: () =>
              Promise.resolve(Buffer.from("fake-image-data")),
          },
    ...
    it("should handle versioning parameter", async () => {
      const s3Url =
        "https://stakr-verification-files.s3.ap-southeast-2.amazonaws.com/verification-files/images/challenge/thumb.webp";
    ...
    it("should reject unauthenticated requests with 401", async () => {
      ...
      const mockRequest = new NextRequest(
        "http://localhost:3000/api/image-proxy?url=" +
          encodeURIComponent(
            "https://stakr-verification-files.s3.ap-southeast-2.amazonaws.com/verification-files/images/x.webp",
          ),
      );
    ...
    it("should handle large image files", async () => {
      const s3Url =
        "https://stakr-verification-files.s3.ap-southeast-2.amazonaws.com/verification-files/images/large-image.jpg";
    ...
    it("should handle network timeouts and retries", async () => {
      const s3Url =
        "https://stakr-verification-files.s3.ap-southeast-2.amazonaws.com/verification-files/images/timeout-test.jpg";
  ...
  describe("Security and Validation", () => {
    ...
    it("should sanitize URL parameters", async () => {
      const maliciousUrl =
        "https://stakr-verification-files.s3.ap-southeast-2.amazonaws.com/file.jpg?/../../../etc/passwd";
