...
jest.mock("@/lib/db/transaction", () => ({
  withTransaction: (work: any) => work(sql),
}));
...
beforeEach(() => {
  ...
  sql.mockImplementation(async (strings: TemplateStringsArray) => {
    ...
    if (text.includes("SELECT id FROM challenge_participants"))
    ...
    if (text.includes("INSERT INTO challenge_participants"))
      return [{ id: "participant", joined_at: "2026-09-16T00:00:00Z" }];
...
function writes() {
  return sql.mock.calls.filter(([strings]) =>
    /INSERT|UPDATE users/.test(strings.join(" ")),
  );
}
...
it("returns the committed balance and numeric participant count, with an audit event", async () => {
  ...
  const audit = sql.mock.calls.find(([strings]) =>
    strings.join(" ").includes("INSERT INTO challenge_audit_events"),
  )!;
  expect(JSON.parse(audit.at(-1))).toMatchObject({
    stakeCents: 5000,
    entryFeeCents: 250,
    entryFeeBps: 500,
  });
...
it.each([
  ...
  "ended",
  "rewards_distributed",
])("blocks %s challenges without writes", async (status) => {
...
it("blocks a duplicate with no extra debit or ledger rows", async () => {
  duplicate = true;
  await expect(joinChallenge("challenge", "user", input)).rejects.toMatchObject(
    { status: 409 },
  );
  expect(writes()).toHaveLength(0);
});
...
it("does not let pointsOnly bypass a stake debit", async () => {
  await expect(
    joinChallenge("challenge", "user", { ...input, pointsOnly: true }),
  ).rejects.toThrow("requires a credit stake");
  expect(writes()).toHaveLength(0);
});

it("rejects XP-only challenges until they have a supported settlement policy", async () => {
...
it.each([{ currency: "CASH" }, "{bad json", [], { currency: "UNKNOWN" }])(
  "fails closed on payment metadata %p",
  async (metadata) => {
...
it("rejects fractional cents before writes", async () => {
  await expect(
    joinChallenge("challenge", "user", { ...input, stakeAmount: 50.001 }),
...
it("propagates ledger failures so the transaction can roll back", async () => {
  ...
  sql.mockImplementation(async (...args) => {
    if (args[0].join(" ").includes("INSERT INTO credit_transactions"))
