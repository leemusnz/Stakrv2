...
jest.mock('@neondatabase/serverless', () => ({ Pool: jest.requireActual('pg').Pool }))
...
describeDatabase('Join PostgreSQL acceptance', () => {
  ...
  beforeAll(async () => {
    ...
    if (!url.pathname.endsWith('_test')) throw new Error('Use a dedicated database ending in _test')
    ...
    `)
    await db.query(readFileSync('migrations/2026-09-16_atomic-joins.sql', 'utf8'))
  })

  beforeEach(async () => {
    await db.query('DROP TRIGGER IF EXISTS fail_write ON credit_transactions')
    await db.query('DROP TRIGGER IF EXISTS fail_write ON challenge_audit_events')
    await db.query('TRUNCATE challenge_audit_events, credit_transactions, challenge_participants, challenges, users CASCADE')
    await db.query('INSERT INTO users (id, credits) VALUES ($1, 100), ($2, 100)', [userA, userB])
    await db.query(`INSERT INTO challenges (id, title, status, start_date, end_date)
      VALUES ($1, 'Test A', 'pending', (now() AT TIME ZONE 'UTC') + interval '1 day', (now() AT TIME ZONE 'UTC') + interval '2 days'),
      ($2, 'Test B', 'pending', (now() AT TIME ZONE 'UTC') + interval '1 day', (now() AT TIME ZONE 'UTC') + interval '2 days')`, [challengeA, challengeB])
  })
  ...
  async function counts() {
    return (await db.query(`SELECT
      ...
      (SELECT count(*)::int FROM challenge_audit_events) audit`)).rows[0]
  }
  ...
  it('commits participant, debit, exact ledger and audit together', async () => {
    ...
    const ledger = await db.query('SELECT sum(amount) amount FROM credit_transactions WHERE user_id = $1', [userA])
    expect(ledger.rows[0].amount).toBe('-52.50')
  })

  it.each(['credit_transactions', 'challenge_audit_events'])('rolls back everything when %s fails', async table => {
    ...
    await expect(joinChallenge(challengeA, userA, input)).rejects.toThrow('injected failure')
    expect(await counts()).toEqual({ participants: 0, ledger: 0, audit: 0 })
    expect((await db.query('SELECT credits FROM users WHERE id = $1', [userA])).rows[0].credits).toBe('100.00')
  })

  it('concurrent duplicate joins debit once', async () => {
    const attempts = await Promise.allSettled([joinChallenge(challengeA, userA, input), joinChallenge(challengeA, userA, input)])
    expect(attempts.filter(r => r.status === 'fulfilled')).toHaveLength(1)
    expect(await counts()).toEqual({ participants: 1, ledger: 2, audit: 1 })
    expect((await db.query('SELECT credits FROM users WHERE id = $1', [userA])).rows[0].credits).toBe('47.50')
  })

  it('concurrent joins cannot oversubscribe the final place', async () => {
    await db.query('UPDATE challenges SET max_participants = 1 WHERE id = $1', [challengeA])
    const attempts = await Promise.allSettled([joinChallenge(challengeA, userA, input), joinChallenge(challengeA, userB, input)])
    expect(attempts.filter(r => r.status === 'fulfilled')).toHaveLength(1)
    expect(await counts()).toEqual({ participants: 1, ledger: 2, audit: 1 })
  })

  it('concurrent joins to different challenges cannot overspend one balance', async () => {
    const attempts = await Promise.allSettled([joinChallenge(challengeA, userA, input), joinChallenge(challengeB, userA, input)])
    expect(attempts.filter(r => r.status === 'fulfilled')).toHaveLength(1)
    expect(await counts()).toEqual({ participants: 1, ledger: 2, audit: 1 })
    expect((await db.query('SELECT credits FROM users WHERE id = $1', [userA])).rows[0].credits).toBe('47.50')
  })

  it.each(['suspended', 'cancelled', 'ended', 'rewards_distributed'])('rejects %s without financial writes', async status => {
    await db.query('UPDATE challenges SET status = $1 WHERE id = $2', [status, challengeA])
    await expect(joinChallenge(challengeA, userA, input)).rejects.toThrow('no longer accepting')
    expect(await counts()).toEqual({ participants: 0, ledger: 0, audit: 0 })
  })

  it('rejects joins at or after start, even if the stored status is pending', async () => {
    await db.query("UPDATE challenges SET start_date = now() AT TIME ZONE 'UTC' WHERE id = $1", [challengeA])
    await expect(joinChallenge(challengeA, userA, input)).rejects.toThrow('when the challenge starts')
    expect(await counts()).toEqual({ participants: 0, ledger: 0, audit: 0 })
  })

  it('floors a fractional fee without balance/ledger drift', async () => {
    const result = await joinChallenge(challengeA, userA, { ...input, stakeAmount: 0.29 })
    expect(result.financial_breakdown).toMatchObject({ entry_fee: 0.01, total_cost: 0.3, remaining_credits: 99.7 })
    expect((await db.query('SELECT sum(amount) amount FROM credit_transactions')).rows[0].amount).toBe('-0.30')
  })
})
