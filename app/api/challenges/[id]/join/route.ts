...
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const result = await joinChallenge(challengeId, session.user.id, validation.data)

    // A committed join must not appear to fail because a nonessential estimate failed.
    let potentialReward: number | null = null
    try {
      potentialReward = validation.data.pointsOnly
        ? await calculatePotentialXPReward(challengeId, session.user.id)
        : await calculatePotentialReward(challengeId, result.participation.stake_amount)
    } catch { console.warn('Reward estimate unavailable after committed challenge join') }
    void triggerAutoSync(challengeId, session.user.id, SYNC_TRIGGERS.CHALLENGE_JOIN)
      .catch(() => console.warn('Auto-sync unavailable after committed challenge join'))

    return NextResponse.json({
      success: true,
      message: 'Successfully joined challenge!',
      ...result,
      financial_breakdown: { ...result.financial_breakdown, potential_reward: potentialReward },
      next_steps: [
    ...
    if (code === '23505') return NextResponse.json({ error: 'You have already joined this challenge' }, { status: 409 })
    if (['55P03', '57014', '40001', '40P01'].includes(code ?? '')) {
      return NextResponse.json({ error: 'Challenge entry is busy. Please retry.' }, { status: 503, headers: { 'Retry-After': '2' } })
    }
...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    try {
      ...
      return NextResponse.json({ error: 'Failed to get participation status' }, { status: 500 })
    }
