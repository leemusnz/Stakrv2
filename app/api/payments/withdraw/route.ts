...
export async function POST(request: NextRequest) {
  if (process.env.ENABLE_CASH_PAYMENTS !== "true")
    return NextResponse.json(
      { error: "Cash payments are not available in the credits MVP" },
      { status: 403 },
    );
  try {
    ...
    const withdrawalTotal = parseFloat(
      recentWithdrawals[0]?.total_amount || "0",
    );

    if (withdrawalCount >= 3) {
      console.warn(
        `⚠️ Rate limit exceeded for user ${session.user.id}: ${withdrawalCount} withdrawals in 24h`,
      );
      return NextResponse.json(
        {
          error: "Withdrawal limit exceeded",
          message:
            "You can only make 3 withdrawals per 24 hours. Please try again later.",
          details: {
            limit: 3,
    ...
    if (withdrawalTotal + amount > DAILY_WITHDRAWAL_LIMIT) {
      console.warn(
        `⚠️ Daily limit exceeded for user ${session.user.id}: $${withdrawalTotal + amount} in 24h`,
      );
      return NextResponse.json(
        {
    ...
    const challengesCompleted = parseInt(
      userInfo[0].challenges_completed || "0",
    );
    ...
    if (accountAgeDays < 7 && amount > 100) {
      ...
      console.warn(
        `🚨 Suspicious: New account (${accountAgeDays} days) withdrawing $${amount}`,
      );
    }
    ...
    if (trustScore < 30 && amount > 50) {
      ...
      console.warn(
        `🚨 Suspicious: Low trust score (${trustScore}) withdrawing $${amount}`,
      );
    }
    ...
    if (falseClaims > 2 && amount > 50) {
      ...
      console.warn(
        `🚨 Suspicious: User with ${falseClaims} false claims withdrawing $${amount}`,
      );
    }
    ...
    if (challengesCompleted === 0 && withdrawalCount === 0 && amount > 50) {
      ...
      console.warn(
        `🚨 Suspicious: User with 0 completed challenges withdrawing $${amount}`,
      );
    }
      ...
      return NextResponse.json(
        {
          ...
          message:
            "Your withdrawal has been flagged for security review. Our team will review your request within 24-48 hours.",
          details: {
      ...
      return NextResponse.json(
        {
          ...
          message:
            "Insufficient funds or concurrent transaction detected. Please try again.",
        },
        { status: 400 },
      );
    }
    ...
    let payoutMessage =
      "Your withdrawal is being processed. Funds will be transferred to your linked bank account.";
    ...
    try {
      ...
      if (stripeSecret && transactionId) {
        ...
        if (payoutResult.success) {
          ...
          console.warn(
            `⚠️ Withdrawal payout failed for user ${session.user.id}: ${payoutResult.error}`,
          );
        ...
        console.warn(
          `⚠️ Stripe not configured or missing transaction ID for user ${session.user.id}`,
        );
      }
    } catch (payoutError) {
      ...
      return NextResponse.json(
        {
          ...
          message:
            "An error occurred while processing your payout. Please contact support.",
          details:
    ...
    return NextResponse.json(
      {
        ...
        message:
          "An unexpected error occurred while processing your withdrawal. Please try again or contact support.",
        details:
          process.env.NODE_ENV === "development"
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      ...
      example_fees: [
        { amount: 10, fee: 0.3, net: 10, total_deducted: 10.3 },
        { amount: 50, fee: 1.5, net: 50, total_deducted: 51.5 },
        { amount: 100, fee: 3.0, net: 100, total_deducted: 103.0 },
        { amount: 500, fee: 15.0, net: 500, total_deducted: 515.0 },
      ],
    });
  } catch (error) {
